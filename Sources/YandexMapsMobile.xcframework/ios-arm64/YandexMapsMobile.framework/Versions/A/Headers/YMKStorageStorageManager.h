...
typedef void(^YMKStorageManagerPathSetterListener)(
    NSError * _Nullable error);
 ...
 * non-cancellable. If there is already a pending operation moveData or
 * setPath, it throws an error (Android). If the application exits
...
/**
 * Sets a new path for storage. If the specified path contains an
 * existing storage, this storage will be used; otherwise, a new storage
 * will be initialized. If there is already a pending operation moveData
 * or setPath, it throws an error (Android).
 */
- (void)setPathWithPath:(nonnull NSString *)path
     pathSetterListener:(nonnull YMKStorageManagerPathSetterListener)pathSetterListener;
